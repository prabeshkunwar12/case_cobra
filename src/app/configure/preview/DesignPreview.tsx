...
import { createCheckoutSesion } from './actions'
...
const DesignPreview = ({configuration}:{configuration: Configuration}) => {
    ...
    const { label: modelLabel } = MODELS.options.find((supportedModel) => supportedModel.value === model)!
    ...
    const { mutate:createPaymentSession } = useMutation({
        ...
        mutationFn: createCheckoutSesion,
        onSuccess: ({ url }) => {
                            ...
                            <div className='flow-root text-sm'>
                                <div className='flex items-center justify-between py-1 mt-2'>
                                    ...
                                    <p className='font-medium text-gray-900'>
                                        {formatPrice(BASE_PRICE / 100, 'USD')}
                                    </p>
                                </div>
                                ...
                                {finish === "textured" ? (
                                    <div className='flex items-center justify-between py-1 mt-2'>
                                        ...
                                        <p className='font-medium text-gray-900'>
                                            {formatPrice(PRODUCT_PRICES.finish.textured / 100, 'USD')}
                                        </p>
                                    </div>
                                ): null}
                                {material === "polycarbonate" ? (
                                    <div className='flex items-center justify-between py-1 mt-2'>
                                        <p className='text-gray-600'>Smooth Polycarbonate Material</p>
                                        <p className='font-medium text-gray-900'>
                                            {formatPrice(PRODUCT_PRICES.material.polycarbonate / 100, 'USD')}
                                        </p>
                                    </div>
                                ): null}
                                ...
                                <div className='flex items-center justify-between py-2'>
                                    ...
                                    <p className='font-semibold text-gray-900'>
                                        {formatPrice(totalPrice/100, 'USD')}
                                    </p>
                                </div>
                            </div>
                        </div>
